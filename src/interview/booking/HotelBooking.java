...
public class HotelBooking {
    ...
    private static List<Room> getRooms(Map<Integer, List<Room>> dataSource, Filter filter) {
        ...
        for (Room room : startDateRoom) {
            if (suitable(featuresFilter, room.features))
                addCommonRooms(dataSource, checkin, checkout, room.availability, featuresFilter, room.features, 0,
                        roomList);
        }
        return roomList;
    ...
    private static void addCommonRooms(Map<Integer, List<Room>> dataSource, int checkin, int checkout, int availabilty,
            Set<String> featuresFilter, Set<String> features, int price, List<Room> roomList) {
        ...
        for (Room nextDay : nextdayRoom) {
            if (suitable(features, nextDay.features)) {
                ...
                addCommonRooms(dataSource, checkin + 1, checkout, new_availabilty, features, new_features, price,
                        roomList);
                price -= nextDay.price;
            }
        }
